    ...
    var body: some View {
        VStack {
            ...
            HStack {
                ...
                WorldDataCard(number: String(format: "%.2f", worldData.fatalityRate), name: "Death (%)", color: .red)
            }
            HStack {
                WorldDataCard(number: worldData.recovered.formatNumber(), name: "Recovered", color: .green)
                WorldDataCard(number: String(format: "%.2f", worldData.recoveryRate), name: "Recovery (%)", color: .green)
            }
        }
        .frame(height: 270)
        .padding()
